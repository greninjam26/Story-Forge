...
This runbook describes the hosted deployment for Story Forge. The services are
created and the application shell is reachable in production; most individual
product flows have not been verified against the hosted deployment.

Three states are tracked separately throughout this document and are not
interchangeable:

- **Committed configuration** — what `render.yaml`, `apps/web/vercel.json`, and
  the Dockerfile declare. Visible in version control.
- **Configured secrets** — values present in the Render and Vercel dashboards.
  Not visible in version control; confirm them in the platform UI.
- **Hosted-verified** — a flow actually exercised against the deployed
  application with authorized test accounts and, where money is involved,
  authorized spending.

Verified on 2026-09-07 by read-only request: the Vercel origin returns `200`
over HTTPS, and `/api/health` returns `{"status":"ok","components":
{"database":"ok"}}` through the Next.js rewrite, so the web shell, the Render
API, and the Neon database are connected end to end. The first request after
idle took 32 seconds on Render's free tier; the next two took under 0.3
seconds. Nothing beyond that is hosted-verified.
...
The committed Blueprint selects real providers, not stubs. `render.yaml` sets:
...
`PAID_TTS_ENABLED=true` means narration calls a paid provider for every story
that reaches the narration stage. Confirm DeepInfra spend limits before
directing anyone to the deployment. Deterministic stubs remain the default for
local development (`apps/api/.env.example`) and for the test suite; that
separation is deliberate, not drift.
...
**These are change procedures, not outstanding activation steps.** The
committed Blueprint already selects Groq text, OpenAI moderation, Cloudflare
images, and DeepInfra narration. Read each section below as the procedure for
switching a provider — to a different one, or back to `stub` as a rollback —
rather than as work still to be done. Where a section says to change a value
"from `stub`", substitute whatever `render.yaml` currently sets.

Change one provider at a time, verify one story, and inspect the cost report
