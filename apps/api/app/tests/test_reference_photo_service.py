...
def test_delete_child_with_reference_photo_removes_child_and_file(
    db_session_factory: sessionmaker[Session],
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_delete_child_with_reference_photo_rolls_back_database_failure(
    db_session_factory: sessionmaker[Session],
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    with db_session_factory() as db:
        ...
        def fail_child_deletion(session: Session) -> None:
            if child in session.deleted:
                raise RuntimeError("database unavailable")
            original_commit(session)
        ...
        with pytest.raises(
            reference_photos.ChildDeletionError
        ) as exc_info:
            reference_photos.delete_child_with_reference_photo(db, child)
    ...
    with db_session_factory() as db:
        saved_child = db.get(Child, child_id)
