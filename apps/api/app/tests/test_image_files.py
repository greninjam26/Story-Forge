...
@pytest.mark.parametrize("image_format", ["JPEG", "PNG", "WEBP"])
def test_normalize_png_returns_resized_metadata_free_png(
    image_format: str,
) -> None:
    image_files = _image_files_module()

    normalized = image_files.normalize_png(
        _image_bytes(image_format, size=(900, 600)),
        max_dimension=511,
    )
    ...
    with Image.open(BytesIO(normalized)) as result:
        ...
        assert result.size == (511, 341)
        assert result.getexif() == {}


@pytest.mark.parametrize("max_dimension", [0, -1])
def test_normalize_png_rejects_non_positive_max_dimension(
    max_dimension: int,
) -> None:
    image_files = _image_files_module()

    with pytest.raises(ValueError, match="max_dimension must be positive"):
        image_files.normalize_png(
            _image_bytes(),
            max_dimension=max_dimension,
        )


def test_normalize_png_rejects_corrupt_bytes() -> None:
    image_files = _image_files_module()
    ...
    with pytest.raises(
        ...
        image_files.normalize_png(b"not an image")


def test_normalize_png_rejects_unsupported_image_format() -> None:
    image_files = _image_files_module()
    ...
    with pytest.raises(
        ...
        image_files.normalize_png(_image_bytes("BMP"))


def test_normalize_png_rejects_animation() -> None:
    image_files = _image_files_module()
    output = BytesIO()
    frames = [
    ...
    with pytest.raises(image_files.InvalidImageError, match="animated"):
        image_files.normalize_png(output.getvalue())


def test_normalize_png_rejects_excessive_source_pixels(
    ...
    with pytest.raises(
        ...
        image_files.normalize_png(_image_bytes(size=(80, 60)))
